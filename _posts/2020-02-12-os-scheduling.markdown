...
배치 처리 시스템은 자료구조 ``큐(queue)``와 유사합니다. 배치 처리 시스템은 **FIFO**, 즉 먼저 들어간 프로그램이 먼저 시작하고 끝납니다. 

(그림)

**배치처리 시스템의 한계**  

한 프로그램의 실행 시간이 너무 길면, 다른 프로그램이 너무 오래 기다려야합니다. 

또한, 한 프로그램을 실행시키면서 **``동시에``** 다른 프로그램도 실행시키고 싶습니다.  

여러 사용자가 동시에 하나의 컴퓨터를 사용하고 싶습니다. (**``다중 사용자 지원``**)
...
#### **멀티 태스킹**
멀티태스킹은 하나의 cpu에서 여러 프로그램이 동시에 실행되는 것처럼 보이게 합니다. 
